...
sheet = read_sheet(
      'https://docs.google.com/spreadsheets/d/1EuQbThPe7XCwU0SkZH9tZFy3_vTdN0vNHORpSTIbwrA/edit?gid=1444641995#gid=1444641995',
      sheet = 'comparison'
)

prompts_gt = sheet |>
      select(subscale, id = `final item ID`, text = `finalized text`) |>
      filter(!is.na(id)) |>
      mutate(
            subscale = subscale |>
                  as_factor() |>
      ...
      group_by(subscale) |>
      gt() |>
      row_group_order(c('cynicism', 'objectivity', 'textbook')) |>
...
compare_gt = sheet |>
      ...
      cols_align(align = 'center', columns = modified)
compare_gt

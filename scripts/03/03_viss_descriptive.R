...
viss_df = select(dataf, prolific_id, starts_with('viss')) |>
      rename_with(.cols = starts_with('viss'), .fn = ~ str_remove(.x, 'viss.'))
...
counts_df = viss_df |>
      pivot_longer(-prolific_id, names_to = 'item', values_to = 'response') |>
      count(item, response) |>
...
counts_df |>
      group_by(item) |>
      mutate(agree = {
            if_else(as.numeric(response) > 3, share, 0) |>
                  sum(na.rm = TRUE)
      }) |>
      ungroup() |>
...
agreement_df = counts_df |>
      ...
      summarize(
            agree_share = sum_if(share, as.numeric(response) >= 4, na.rm = TRUE)
      ) |>
      ungroup() |>
...
topbottom_gt = topbottom |>
      ...
      gt() |>
      cols_label(
            agree_share = md('**agreement**'),
            item = md('**item**'),
            prompt = md('**prompt**')
      ) |>
      fmt_percent(agree_share, decimals = 0) |>
      tab_style(
            cell_fill(color = 'blue', alpha = .25),
            cells_body(rows = group == 'top 5')
      ) |>
      tab_style(
            cell_fill(color = 'red', alpha = .25),
...
## COI ----
coi_plot = {
      ggplot(viss_df, aes(coi.1, coi.2)) +
      ...
      ggMarginal(type = 'histogram', binwidth = 1, fill = 'transparent')
coi_plot
...
## VFI and critiques ----
vfi = exprs(nonsubj.1, nonsubj.2, vfi.1, vfi.2)
crits = exprs(aims.2, ir, stdpt, value.conflict)
...
ggplot(viss_df) +
      ...
      facet_matrix(rows = vars(!!!vfi), cols = vars(!!!crits))
...
list(
      ...
      b = ggplot(viss_df, aes(vfi.1)) +
            geom_bar(),
      c = ggplot(viss_df, aes(ir)) +
            geom_bar() +
            coord_flip()
) |>
      wrap_plots(
            design = focal_design,
            # design = c(area(2, 1), area(1, 1), area(2, 2, 2.5, 2.1)),
            axes = 'collect'
      )

ggsave(here(out_dir, '03_focal.png'), height = 5, width = 5, bg = 'white')

## How many respondents accept both vfi.1 and ir?
viss_df |>
      count(vfi.1 > 4, ir > 4) |>
      mutate(share = n / sum(n))
...
cor_ci = function(col1, col2, data = viss_df) {
      ...
      cor.test(col1_vec, col2_vec) |>
            broom::tidy() |>
            mutate(col1 = as_label(col1), col2 = as_label(col2)) |>
...
coss_plot = function(col_vars) {
      dataf |>
            ...
            facet_matrix(rows = vars(coss), cols = vars(!!!col_vars))
}
coss_plot(vfi)
coss_plot(crits)
...
coss_plot(exprs(vfi.1, ir)) +
      trust_hist_gg +
      plot_layout(axes = 'collect', widths = c(2, 1))

ggsave(here(out_dir, '03_trust_focal.png'), height = 4, width = 8, bg = 'white')

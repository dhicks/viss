...
homophones = c('aims.1', 'aims.3', 'coi.1', 'consensus.2', 'nonsubj.1')
...
dataf_1 = here('data', '01', 'data.Rds') |>
      ...
      mutate(study = 1L) |>
      rename_with(
            .cols = any_of(c(homophones, names(renumber))),
            .fn = ~ str_c(.x, '-1')
      ) |>
...
agree_1 = dataf_1 |>
      ...
      summarize(across(
            !prolific_id,
            .fns = list(
                  agree = ~ mean(.x > 3, na.rm = TRUE),
                  n = ~ sum(!is.na(.x))
            ),
            .names = '{.col}&{.fn}'
      )) |>
      pivot_longer(
            cols = c(ends_with('&agree'), ends_with('&n')),
...
agree_3 = dataf_3 |>
      ...
      summarize(across(
            !prolific_id,
            .fns = list(
                  agree = ~ mean(.x > 4, na.rm = TRUE),
                  n = ~ sum(!is.na(.x))
            )
      )) |>
      pivot_longer(
            cols = c(ends_with('_agree'), ends_with('_n')),
            names_to = c('item', 'stat'),
            names_sep = '_',
            values_to = 'value'
      ) |>
...
dataf = bind_rows(agree_1, agree_3) |>
      mutate(
            moe = qnorm(.975) * sqrt(agree * (1 - agree) / n),
            ci.lo = agree - moe,
            ci.hi = agree + moe,
...
agree_gg = ggplot(
      dataf,
      aes(fct_rev(item), agree, color = study, shape = study)
) +
      geom_pointrange(
            aes(ymin = ci.lo, ymax = ci.hi),
...
used_gg = dataf |>
      ...
      ggplot(aes(study, fct_rev(item), color = study)) +
      # geom_raster() +
...
agree_gg +
      used_gg +
      plot_layout(widths = c(5, 1), axes = 'collect', guides = 'collect') &
      theme(legend.position = 'bottom')

ggsave(
      here('out', '03', '07_desc_comb.png'),
      height = 7,
      width = 7,
      scale = 1,
      bg = 'white'

...
dataf |>
      ...
      ggplot(aes(institution, fill = confidence)) +
...
corr_gg = ggplot(dataf) +
      ...
      facet_matrix(vars(!!trust_vars), layer.diag = 3)
corr_gg

corr_gg + as_gtable(corr_gt) + plot_layout(ncol = 1, heights = c(3, 1))

ggsave(
      here(out_dir, '05_trust_corr.png'),
      height = 6,
      width = 6,
      bg = 'white',
      scale = 1.25
...
ggplot(dataf) +
      ...
      facet_matrix(rows = vars(!!trust_vars), cols = vars(!!demo_cont))
...
ggplot(dataf) +
      geom_autopoint(alpha = .1, position = 'jitter') +
      stat_smooth(aes(.panel_x, .panel_y), method = 'lm') +
      facet_matrix(rows = vars(!!trust_vars), cols = vars(items[1:10]))
ggplot(dataf) +
      geom_autopoint(alpha = .1, position = 'jitter') +
      stat_smooth(aes(.panel_x, .panel_y), method = 'lm') +
...
inter_gg = dataf |>
      mutate(obj_binned = cut(objectivity, breaks = 4)) |>
      ggplot(aes(
            cynicism,
            coss,
            color = obj_binned,
...
inter_reg_gg = predict_response(
      model_2,
      c('cynicism', 'objectivity [1:7 by=2]'),
      margin = 'empirical'
) |>
      plot() +
...
ggsave(
      here(out_dir, '05_interaction.png'),
      height = 3,
      width = 6,
      scale = 1.5,
      bg = 'white'
...
rwa_fit = lm(
      coss ~ rwa.conservatism + rwa.traditionalism + rwa.authoritarianism,
      data = dataf
)
summary(rwa_fit)
...
lst('rwa.conservatism', 'rwa.traditionalism', 'rwa.authoritarianism') |>
      map(~ predict_response(rwa_fit, terms = .x, margin = 'empirical')) |>

...
The aim of the project reported here was to develop a Values in Science Scale (VISS) that could be used to measure public views on the relationship between science, values, and policy, and thereby test these kinds of claims made by HPSTS scholars.  In the next section we provide some additional motivation for the project, and discuss the initial development of VISS. We then proceed through the three empirical studies, and conclude with a more philosophical general discussion. 
...
To ensure construct validity at the individual item level, we ended up using a mix of quantitative and qualitative methods across the three studies reported here. Study 1 was an initial exploration, using all of the prompts on the original list compiled by [author1] and quantitative methods of exploratory and confirmatory factor analysis. As we discuss below, some of the results of this study suggested potential construct validity issues. Study 2 used qualitative methods, involving all four authors, to check the way respondents were interpreting the prompts and revise their wording to ensure construct validity. Study 3 then returned to quantitative EFA. 
...
A technical presentation of factor analysis results is given in @sec-study1-fa. Briefly, we examined exploratory factor analysis (EFA) models with three and six latent factors, and focused on the six-factor model as being more easily interpretable. This model covered 29 out of 36 VISS items, and with factors that we labeled as *scientism*, *values in science*, *cynicism*, *power* dynamics within the scientific community, a simplistic *textbook* picture of scientific practice, and the *value-free ideal*. We then turned to confirmatory factor analysis (CFA) of these six factors, using a holdout subset of the data. Unfortunately, quantitative goodness-of-fit measures for the CFA were poor. 


## Discussion

This initial EFA study identified six potential factors covering 29 out of the original 36 VISS items.  Most of these factors had clear and intriguing interpretations, such as scientism, cynicism about science, a simplistic "textbook" picture of how science works, and the value-free ideal. 

However, a quantitative check of this six-factor structure using CFA indicated poor fit.  Qualitative review also raised concerns about construct validity.  For example, a factor labelled "cynicism" covered a number of items expressing a cynical attitude towards science, with loaded items related to conflicts of interest, the idea that "The consensus of the scientific community is based on social status and prestige rather than evidence," and the idea that "When a scientific theory changes or is revised, it means that the research that went into it initially was flawed."  However, this factor also had the item "Standards of scientific evidence are different in different situations."  We had intended this item to articulate the argument from inductive risk [@DouglasSciencePolicyValuefree2009; @HavstadSensationalScienceArchaic2021], that is, the idea that a hypothesis with serious downstream risks if it turns out to be false — such as a claim that a widely-used pesticide does not cause non-Hodgkins lymphoma — should require significantly more evidence than a hypothesis without such downstream risks — such as a claim about the durability of a certain brand of hiking shoe.  In other words, different standards of evidence can be *appropriately* different in different situations.  
...
For this study, items were arranged into 3 blocks of 5-7 items each, roughly corresponding to latent variables identified in the study 1 EFA, e.g., a "cynicism" block, a "textbook" block.  In Qualtrics, we prepared a survey in which participants were randomly allocated to a single block.  Participants were presented with the item text, a 5-point agree/disagree Likert scale, and a free text response box, with the instruction to "Please indicate how much you agree with each of the following statements. Then, in the text box, please write a few sentences explaining why you answered the way you did." (Between the first and second iterations of data collection — explained below — the Likert scale was expanded from 5 to 7 points to facilitate more nuanced comprehension of the items.)
...
Data collection and analysis was conducted iteratively: responses were collected, then analyzed, and prompts determined to be either "saturated" (all respondents understood the prompts as intended) or in need of revision, with the next iteration using the revised prompts. Ultimately, 5 iterations were run, all during March 2024.  
...
@fig-vfi shows marginal distributions for these VFI and critiques items, along with scatterplots and correlation coefficients for each VFI-critique pair.  Starting with the marginal distributions, we see there is majority agreement with both nonsubj items, and vfi.1 (VFI items) and aims.2, ir, stdpt (critiques); and majority rejection of vfi.2 and value.conflict (compare @fig-03-likert).  
...
In short, at the individual item level, there is a notable *absence* of association between the VFI and its critiques, on the one hand, and trust in science, on the other. 
...
Based on the loadings and factor interpretation, the most theoretically coherent EFA solution comprised three factors: textbook, cynicism, and objectivity. 
...
# Study 1 factor analysis {#sec-study1-fa}

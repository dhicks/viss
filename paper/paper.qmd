...
*[Add VFI introduction here
we're especially interested in the value-free ideal, which is how philosophers understanding traditional objectivity

@DouglasSciencePolicyValuefree2009: social, ethical, and political values should have no influence over the reasoning of scientists, and that scientists should proceed in their work with as little concern as possible for such value (1)
]*
...
Anonymized original data and analysis code are available at <https://github.com/dhicks/transparency>.  


# Study 1

Study 1 was conducted concurrently with the replication study reported in [redacted <!-- @HicksValuesDisclosuresTrust2022 -->].  Analysis of individual items and exploratory factor analysis (EFA) uncovered some intriguing patterns, but exhibited poor fit statistics in confirmatory factor analysis (CFA).  
...
here('paper', 'tbl', '06_prompts.Rds') |>
    read_rds() |>
    ## So for some reason this modifies the width of text
    cols_width(id ~ pct(75))
```
...
One alternative explanation for the observed nearly flat relationship between objectivity and generalized trust in science might be a potential interaction effect between objectivity and cynicism.  Specifically, members of the public who endorse VFI might have very different views of whether scientists generally comply with VFI.  When a respondent endorses VFI and believes that scientists generally comply with it, this explanation would predict high trust.  While a respondent who endorses VFI and believes scientists generally do *not* comply with it would be predicted to have *low* trust.  At the other end of the objectivity scale, respondents who reject VFI *[finish; which plot to show?]*

In the current VISS, the cynicism factor might be used as a rough proxy for VFI compliance.  This factor focuses on a limited set of values, namely, financial conflicts of interest.  The influence of these values is a sufficient condition for violating VFI, but not necessary; participants might believe that conflicts of interest are not a problem in science (low cynicism) but still believe that VFI is violated by other kinds of value influences.  

The interaction between objectivity and cynicism predicted by this alternative explanation does not appear to be supported by the study 3 data. @tbl-inter shows coefficients for two regression models of CoSS against the FA latent variables from the 3-factor model, respectively, without and with an interaction term between objectivity and cynicism.  The point estimate for the effect of the interaction term is negligible and not statistically significant; the first-order coefficients are all unchanged between specifications; and adjusted $R^2$ and AIC (very, very slightly) favor the non-interacted model.  
...
The left panel of figure @fig-inter shows a conditional scatterplot and univariate regression of CoSS against objectivity scores, stratified by cynicism quartile.  This descriptive plot does not suggest much heterogeneity in the effect of objectivity on generalized trust.  The right panel shows "empirical," "counterfactual," or average predicted values of CoSS [@DickermanCounterfactualPredictionNot2020], across values of cynicism and objectivity.  These estimates were calculated from the interaction regression model (model 2 from @tbl-inter) using the `ggeffects` package version 1.5.2 [@LudeckeGgeffectsCreateTidy2024].  If the alternative explanation were correct, we would expect to see very different slopes in the marginal estimates

![Conditional scatterplot and estimated marginal effects for an objectivity-cynicism interaction. In both panels, x-axis corresponds to objectivity latent factor score, y-axis corresponds to CoSS score, and color corresponds to cynicism latent factor score.  Left: scatterplot and separate univariate regressions of CoSS against objectivity, conditional on cynicism quartile.  Right: Empirical/"counterfactual"/average predicted values of CoSS, calculated from model 2 from @tbl-inter.](img/05_interaction.png){#fig-inter}
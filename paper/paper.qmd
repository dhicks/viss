...
abstract: "Scholars in the history, philosophy, and social studies of science often explain public scientific controversies by appealing to views of the general public on appropriate roles for values in science. However, evidence that members of the public actually hold these views is limited. This paper presents the Values in Science Scale (VISS), a quantitative survey instrument designed to measure these views. In some notable initial findings using VISS, we find that members of the public are fallibilists and recognize the role of knowledge-based consensus in science; that they endorse both the value-free ideal (VFI) and some influential philosophical critiques of it; and that cynicism about science, rather than VFI, is a key predictor of generalized (dis)trust in science." 

# acknowledgments: "For feedback on this project, thanks to participants at the 10th Annual Values in Medicine, Science, and Technology Conference at UT Dallas, May 2022, and the Knowledge, Science, and Society conference at UC San Diego, May 2024. In particular, thanks to Magdalena Grohman for suggesting the qualitative approach used in study 2, Sarah Depaoli for feedback and advice on our use of factor analysis, and Matthew Brown for significant feedback on a draft.  Thanks to UC Merced for funding to support EJCL, CC, and JD throughout different stages of this project, as well as funding for the survey studies."
...
<!-- BJPS submission guidelines: https://www.journals.uchicago.edu/journals/bjps/instruct
    - strict 24-page limit. This includes footnotes and appendices but excludes title, abstract, and reference list
    - Times New Roman
    - 12 point
    - 1.5 line spacing
    - 1 inch margins
- abstract of no more than 300 words
- anonymise, hide acknowledgments

- artwork instructions: https://www.journals.uchicago.edu/journals/bjps/prep-art
 -->
...
The paper proceeds as follows. In @sec-dev we discuss the initial development of the survey instrument, including the first two empirical "developmental" studies. @sec-study3 reports the findings of the third empirical, "validation" study, including findings for individual survey items (@sec-individual, @sec-vfi) and exploratory factor analysis (@sec-fa, @sec-fa-trust). To keep the main text as accessible as possible for a general philosophical audience, detailed discussion of the study design and statistical analyses are provided in supplemental sections (@sec-supp-study1, @sec-supp-study2, and @sec-supp-study3). @sec-discussion and @sec-conclusion offer an interpretation of these findings in the context of the HPSTS literature. 

# Development of VISS items {#sec-dev}

In this section, we first discuss the initial drafting of the VISS items (@sec-dev-initial) and then briefly report the results of the first two empirical studies (@sec-study1, @sec-study2). 

## Initial drafting {#sec-dev-initial}

As a survey instrument for use with the general public, we wanted the Values in Science Scale to comprise a series of accessible, clearly-written prompts, with a Likert-style quantitative response (a 5- or 7-step agree/disagree scale). 
...
We emphasize that the VISS prompts are theoretically-informed but were not developed to capture predesignated latent factors.  For example, the three "aims of science" prompts state that the aims of science are knowledge for its own sake, the protection of human health, and economic growth.  We did not necessarily expect responses on these items to be correlated, and so did not expect exploratory factor analysis to indicate that they form an "aims of science" latent variable.  Consequently, some common elements of psychometric measure development were not relevant to this particular project [@DepaoliSelectingDevelopingAnalyzing2025]. 
...
We conducted three empirical studies to develop and begin to validate the VISS, using a mix of quantitative and qualitative methods. To keep this paper accessible for a philosophical audience, we provide a detailed discussion of the methods and statistical analysis for each study in the supplement (@sec-supp-study1; @sec-supp-study2; @sec-supp-study3).  Here we briefly discuss studies 1 (@sec-study1) and 2 (@sec-study2); study 3 is discussed in @sec-study3. 

Study 1 was an initial exploration, using all of the prompts on the original list compiled by [author1] and quantitative methods of exploratory (EFA) and confirmatory (CFA) factor analysis. Some of the results of this study suggested potential construct validity issues. Study 2 used qualitative methods to check the way respondents were interpreting the prompts and revise their wording to ensure construct validity. Study 3 then returned to quantitative EFA. 
...
```{r}
#| label: tbl-viss
#| tbl-cap: "Finalized VISS items" 
#| fig-pos: "t"
here('paper', 'tbl', '06_prompts.Rds') |>
      read_rds() |>
      cols_width(id ~ pct(20), text ~ pct(60)) |>
      tab_source_note(
            'Subscales are based on study 3 EFA, 3-factor solution. aims.1 is reverse-coded in the cynicism subscale.'
      ) |>
      tab_options(latex.use_longtable = TRUE)
```


# Validation (Study 3) {#sec-study3}
...
The value-free ideal (VFI) plays a significant role in a number of explanations of public scientific controversies put forward by philosophers of science.  VISS contains four items that correspond to the value-free ideal — nonsubj.1 and .2 and vfi.1 and .2.  The two nonsubj items concern the role of subjectivity in research, and in particular reject "imagination or creativity" (nonsubj.1) and "interpretation" (nonsubj.2). vfi.1 is a statement of VFI as usually defined by philosophers: "The evaluation and acceptance of scientific results must not be influenced by social and ethical values." And vfi.2 rejects the use of "imagination or creativity" in science. 

VISS also contains four items corresponding to philosophical critiques of the value-free ideal. aims.2 corresponds to the "aims approach" to values in science, which argues that science has non-epistemic aims — such as protecting human health and the environment — and that such aims can legitimately influence all aspects of research [@ElliottNonepistemicValuesMultiple2014; @IntemannDistinguishingLegitimateIllegitimate2015; @PotochnikIdealizationAimsScience2017; @HicksWhenVirtuesAre2022].  ir corresponds to the argument from inductive risk.  According to this argument, because and insofar as science has the potential to cause harmful social consequences, it's appropriate to require greater evidence (or allow weaker evidence) based on the severity of those consequences [@DouglasSciencePolicyValuefree2009; @HavstadSensationalScienceArchaic2021].  stdpt corresponds to standpoint epistemology, a theory associated with feminist, Marxist, and critical race analyses of science [@CrasnowFeministStandpointTheory2014].  According to standpoint epistemology, members of marginalized groups can be in a position to better identify and understand oppressive social phenomena than members of privileged groups.  
...
The most consistent factor, occurring in some form across all six models, was a *textbook* perception of science:  based on unanimity (consensus.2), the idea that established science is fixed (fallible.1), and the use of laboratory experiments (pluralism.1).  The second consistent factor was *cynicism*, which also occurred across all six models; textbook and cynicism items appeared together in the 1-factor model.  The cynicism factor included concerns about conflicts of interest (coi.1 and coi.2) and the idea that scientific consensus is non-epistemic (consensus.1).  For models 2 and 3, this factor also had negative loadings for the idea that science has epistemic primary aims (aims.1).  A third common factor was *objectivity*, with three versions of VFI (nonsubj.1, nonsubj.2, vfi.1). 

Models 4-6 also included a factor comprising claims about the aims of science (aims.1, aims.2) and the central claim of standpoint epistemology (stdpt).  It's not clear to us how to interpret these factors.  Models 5 and 6 also include two singleton factors, for pluralism.2 ("same strict requirements") and the argument from inductive risk (ir). 

In the rest of the analysis reported in the main text, we focus on the 3-factor model. Distributions of scores for each factor are shown in the top row of @fig-factors-coss.

![Marginal distributions and scatterplots for discretized scores from the 3-factor model (x-axes, upper panel) against CoSS (y-axis, right panel). Lines in each scatterplot are univariable regressions with 95% CIs. Point positions in scatterplots are jittered for legibility. P-values are calculated for the null that $R = 0$, not adjusted for multiple comparisons.](img/05_factors_coss.png){#fig-factors-coss}

We can interpret the textbook and objectivity scores using the same 7-step disagree-agree scale as the individual items: 4 is the midpoint, 1 is maximal disagreement, 7 is maximal agreement. In @fig-factors-coss, the vast majority of respondents reject the simplistic, textbook picture of science; while a smaller majority accept VFI. 

Cynicism is slightly more complicated, because one of the five items has a negative weight. The midpoint here is $\frac{1}{4}(3 \times 4 + -4) = 2 \times 4 / 4 = 2$; using a similar calculation, the maximum is 5 and the minimum -1. Only a small minority have a cynicism score greater than 2. 

All together, while objectivity is a majority view, textbook and especially cynicism are minority views. (Discussion of @fig-factors-coss continues below.)
...
![Coefficients (point estimates and 95% confidence intervals) for regression models of generalized trust (CoSS) against latent VISS factors.  Factors use the interpretive labels with the model index, e.g., "cynicism_5" is the *cynicism* factor from the 5-factor model.  Dashed lines indicate ±1/7 and 0.  Latent variables scores were calculated as signed averages of component variables (each using a seven-point Likert scale), so that a one-point difference in a latent variable corresponds to an average difference of one point in the component variables. CoSS is calculated as an average of variables that use a seven-point Likert scale.](img/04_trust_coefs.png){#fig-trust-coefs width=50%}

(Readers might notice that the association for objectivity is statistically significant in @fig-factors-coss [$p = .004$] but the confidence interval for objectivity_3 in @fig-trust-coefs overlaps with 0 and so is not statistically significant. This is the result of multivariable regression: discretized scores for objectivity and textbook are correlated in EFA model 3, and this correlation confounds the univariable objectivity-CoSS regression [@MorrisseyMultipleRegressionNot2018].)

Using the ±1/7 threshold, the regression models find a robust and practically significant effect only for cynicism.  The estimates for textbook are robustly compatible [@RafiSemanticCognitiveTools2020] with a practically significant negative effect; but these are only statistically significantly different from -1/7 for the first three FA models.  Similarly, the estimates for the uninterpreted "aims and standpoint" factor are compatible with a positive effect, but only significantly different from 1/7 for a single FA model.  Notably, the objectivity factor is almost never compatible with a practically significant effect; in other words, there was a robust *lack* of correlation between the objectivity factor and CoSS. 



# General Discussion {#sec-discussion}

This project began to develop the Values in Science Scale, an instrument for assessing views of the public on roles for values in science.  Factor analysis found theoretically coherent clusters of items that could be treated as subscales; while analyses among VISS items and between subscales and measures of generalized trust in science empirically enrich theoretical discussions in philosophy and other fields.  
...
In study 3, we found that there was no practically significant correlation between versions of the value-free ideal — the traditional conception of objectivity — on the one hand, and philosophical criticisms of this ideal, on the other. Indeed, a number of respondents indicated that they agreed with both VFI and a critique.  

It may be tempting to explain this as irrationality: these respondents have failed to realize that they hold inconsistent beliefs. Some social psychologists have argued that humans are generally irrational, that deeply-held convictions are driven by accidents of group identity [@KahanCulturalCognitionScientific2011] and arational emotional responses to stimuli [@HaidtEmotionalDogIts2001], and that justifications for one's belief — reasons — are post hoc rationalizations that primarily function to influence other people [@HaidtEmotionalDogIts2001 fig. 2; @MercierEnigmaReason2017]. On these accounts, inconsistent beliefs would be expected as the product of changing group circumstance, inconsistent emotional responses, and ad hoc reactions to challenges. Some conservative-libertarian political writers have leveraged this empirical research to argue against democracy [for summaries and critical responses see @ReissExpertiseAgreementNature2019; @FarrellAnalyticalDemocraticTheory2023]. 

Here we sketch an alternative interpretation, drawing on @KleinDataFrameTheory2007 and @HalpernScienceExperienceDeweyan2022 and more generally inspired by the work of philosopher John Dewey.  
...
As tools, frames can be used deliberately (as by a careful novice or in an especially challenging application) or automatically (as by an overconfident novice, or by an experienced wielder in ordinary use). They can also be misapplied, as indicated by the aphorism (itself a frame) that "when all you have is a hammer, everything looks like a nail." Specifically, if all many scientists have is VFI, then every interaction between "science" and "the social" one happens to be concerned with — regulatory capture, diversity and inclusion initiatives, agencies canceling grants based on crude keyword searches — looks like a violation of value-freedom. 

Critically, frames might be context-specific, deployed differently in different kinds of situations.  For instance, during the first Trump administration, several scientific and advocacy organizations deployed aims.2 — the idea that science aims to protect human health and the environment — to challenge a proposed open science rule at the US Environmental Protection Agency [@HicksWhenVirtuesAre2022; [redacted]<!--ST text mining-->].  Then, during the Biden administration, many of these same organizations supported an approach to "scientific integrity" that evokes vfi.1 and nonsubj.1 (<https://www.whitehouse.gov/wp-content/uploads/2023/01/01-2023-Framework-for-Federal-Scientific-Integrity-Policy-and-Practice.pdf>).  At the level of abstract principle these positions are logically incompatible.  But because they're not activated in the same context, there is no "felt doubt" or practical inconsistency.  

Working with this Deweyan conceptual framework, one important area for future research will be to understand which tropes are deployed as people navigate different public scientific controversies, and under what circumstances agents might recognize that they are deploying two logically incompatible tropes, that is, when a logical inconsistency becomes a practical inconsistency. (Compare Farrell et al.'s call "to investigate the conditions under which democratic problem solving is likely to succeed or fail" [-@FarrellAnalyticalDemocraticTheory2023 770] as a response to the conservative-libertarian social psychological arguments against democracy.) 


## Values in science and generalized trust

The patterns of correlation in @fig-factors-coss, @fig-trust-coefs, and @fig-trust challenge the common idea that VFI provides a foundation for trust in science.  Indeed, our findings indicate that trust in science is generally independent of VFI.  This is in line with @HicksValuesDisclosuresTrust2022, who found that disclosing public health values appeared to increase, rather than decrease, trust in the disclosing scientist. 

In contrast, cynicism was consistently negatively correlated with trust.  We note that, given cynicism, reduced trust in science would be warranted.  That is, insofar as it were the case that science was rife with conflicts of interest, prestige effects, and flawed research, then science would be less trustworthy.  

These findings suggest that objectivity — the value-free ideal — does not play a role in general trust in science, and instead that cynicism — concerns about things like conflicts of interest — is more important. In @sec-obj-cyn we examine an alternative explanation, a potential interaction effect between objectivity and cynicism, that maintains a role for VFI in trust in science. However, we find no evidence to support this alternative in the available data. 
...
# Conclusion: Implications for science communication {#sec-conclusion}

In response to a perceived crisis of trust in science, science communicators, practicing scientists, and others have argued for interventions that stress scientific fallibilism, the practical benefits of science, and compliance with the value-free ideal [@LupiaTrendsUSPublic2024; @ThorpTeachPhilosophyScience2024]. These arguments assume that (a) there is a widespread lack of trust in science, and (b) public misunderstanding of the philosophy of science and/or (perceived) violations of VFI are important drivers of this loss of trust. 

While the present study did not simulate a science communication context, our results challenge both assumptions. 

In study 3, generalized trust in science was moderately high (e.g., for CoSS, mean 4.6, median 5.0, on a 1-7 scale); this is in line with larger and more rigorous surveys of generalized trust in science [@LupiaTrendsUSPublic2024; @Veckalov27countryTestCommunicating2024]. In the US, there is a long-term partisan divergence, with conservatives/Republicans having slightly lower levels of trust in science than liberals/Democrats [@TysonPublicTrustScientists2024]. But this is the difference between a supermajority (~65% of Republicans expressing "a fair amount" or "a great deal" of trust in scientists) and near-consensus (~90% of Democrats ). Many examples used to illustrate the purported widespread lack of trust in science — such as the recent rise of vaccine skepticism — are probably better analyzed as specific groups having specific concerns (potentially founded in misinformation) about specific areas of scientific research and development. 

Regarding the second claim — the role of public misunderstanding of philosophy of science — our findings indicate that, in certain respects, members of the general public may have more sophisticated views than are often attributed to them. The simplistic "textbook" picture of science is not a majority view. Supermajorities of the public appear to recognize fallibilism, the role knowledge-based consensus, and distinguish different kinds of practical aims; and majorities accept certain roles for values. 

At the same time, the traditional, value-free conception of objectivity appears to be widely held. @FernandezPintoLegitimizingValuesRegulatory2019 argue that VFI is "self-undermining" in the context of regulatory science, because there are always violations of VFI that industry interests can use as excuses to delay regulatory action. However, regulatory delay is not the same thing as public distrust, and the available empirical evidence suggests that violations of value-freedom do not as such reduce trust in science. VFI as an interpretive frame may also be unstable or highly context-dependent, since majorities of our respondents also endorse logically incompatible, value-laden views about science. 

On the other hand, our findings suggest that cynicism — concern about conflicts of interest and prestige hierarchies — is a strong predictor of distrust in science. And the specific concerns of specific groups of people about specific areas of research are often based on exactly these kinds of concerns. Therefore, addressing cynicism might be an important avenue for bolstering trust in science. The role of science communication here depends on whether and to what extent this cynicism is grounded in reliable information or misinformation. Science communication will be important for calling out and correcting cynical misinformation; but addressing well-founded cynicism would seem to require, e.g., reforming funding structures to reduce conflicts of interest, rather than a better communications strategy. 
...
Study 1 was conducted concurrently with [redacted <!-- @HicksValuesDisclosuresTrust2022 -->].  Analysis of individual items and exploratory factor analysis (EFA) uncovered some intriguing patterns, but exhibited poor fit statistics in confirmatory factor analysis (CFA).  
...
Throughout both the main text and supplement, we work with discretized or "non-refined" scores [@DiStefanoUnderstandingUsingFactor2009], meaning each item associated with a factor counts the same, i.e., the score is a simple unweighted average of the associated items. (Because aims.1 has a negative loading on cynicism in this factor, its values are negated, i.e., it is weighted -1 in the average.) 
...
### No evidence for an objectivity-cynicism interaction {#sec-obj-cyn}

One alternative explanation for the lack of correlation between objectivity and trust in science might be a potential interaction effect between objectivity and cynicism.  Specifically, members of the public who endorse VFI might have a range of views of whether scientists generally *comply* with VFI.  When a respondent endorses VFI and believes that scientists generally violate it, this explanation would predict *lower* trust.  On the other hand, respondent who endorses VFI and believes scientists generally comply with it would be predicted to have *higher* trust.  At the other end of the objectivity scale, respondents who reject VFI would be indifferent to VFI violations.  In other words, we would expect to see heterogeneous effects of objectivity (VFI acceptance) on trust conditional on (perceived) VFI violations; see @fig-inter-model. Then, averaging across (perceived) VFI violations, the positive effect of objectivity on one end could "cancel out" the negative effect on the other hand, with no overall effect of objectivity on trust. 

![Abstract interaction model between objectivity and trust in science.](img/interaction.pdf){#fig-inter-model width=40%}

This alternative explanation, if supported empirically, would be compatible with both the regression analysis reported in the main text and the common assumption that VFI is important for trust in science.  
...
All together, the objectivity-cynicism interaction model is not supported here.  
